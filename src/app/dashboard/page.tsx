...
export default async function DashboardPage() {
  ...
  // Fetch quick metrics & documents for analysis
  const [banksCount, usersCount, templatesCount, cheques, effets, banks] = await Promise.all([
    prisma.bank.count(),
    prisma.user.count(),
  ...
  const effetsCount = effets.length;
  ...
  const bankAnalysis = banks.map((b) => {
    const bCheques = cheques.filter((c) => c.bankId === b.id);
    const bEffets = effets.filter((e) => e.bankId === b.id);
