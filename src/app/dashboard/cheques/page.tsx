...
export default function ChequesPage() {
  ...
  const fetchCheques = async () => {
    ...
    try {
      ...
      if (!res.ok) throw new Error("Erreur de chargement des chèques.");
  ...
  const handleRestoreCheque = async (id: string) => {
    ...
    setSuccess(null);
    try {
      const res = await fetch(`/api/cheques/${id}/restore`, { method: "POST" });
      const data = await res.json();
  ...
  const handleHardDeleteCheque = async (id: string) => {
    ...
    setSuccess(null);
    try {
      const res = await fetch(`/api/cheques/${id}?hard=true`, { method: "DELETE" });
      const data = await res.json();
  ...
  const handleEditSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(`/api/cheques/${selectedCheque.id}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          bankId,
  ...
  const openPrintPdfModal = async (id: string) => {
    ...
    setIsPrintOpen(true);
    try {
      const res = await fetch(`/api/cheques/${id}/print`);
      if (!res.ok) {
        const data = await res.json().catch(() => ({}));
